...
export class DatasetViz {
  ...
  async redraw(word, uid) {
    ...
    let divIndex;
    for (divIndex = 0; divIndex < this.container.length; ++divIndex) {
      if (this.container[divIndex].getAttribute('word') === word) {
        break;
      }
    }
    if (divIndex === this.container.length) {
      throw new Error(`Cannot find div corresponding to word ${word}`);
    }
    const wordDiv = this.container[divIndex];
  ...
  updateButtons_() {
    ...
    for (const element of this.container) {
      ...
      if (exampleCount === 0) {
        button.textContent = `Samples (${exampleCount})`;
      } else {
        const pos = this.navIndices[word] + 1;
        button.textContent = `Samples (${pos}/${exampleCount})`;
      }
    }
    ...
    if (minCountByClass >= requiredMinCountPerClass) {
      this.startTransferLearnButton.textContent = 'Train model';
